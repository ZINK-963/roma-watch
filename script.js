...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  if (topHeader) {
    const onScroll = () =>
      topHeader.classList.toggle("scrolled-top-header", scrollY > 50);
  ...
  function cardHTML(p, homepage = false) {
    ...
    return `
      <article class="product-card" data-product-id="${p.id}" tabindex="0">
        ...
        <div class="image"><img src="${esc(p.image || "./images/watch2.png")}" alt="${esc(p.brand + " " + p.name)}"></div>
  ...
  async function loadProducts() {
    try {
      ...
      [
        ...
        if (c)
          c.innerHTML =
            '<div class="no-products"><p>خطا در بارگذاری محصولات</p></div>';
      });
    }
  }
